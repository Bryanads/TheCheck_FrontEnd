...
// Tooltip customizado (sem alterações)
...
export const HourlyForecastCharts: React.FC<HourlyForecastChartsProps> = ({ allHoursData, spotlightHour, onClose }) => {
    ...
    return (
        <div className="bg-slate-800/70 backdrop-blur-sm rounded-lg p-4 mt-4 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                ...
                {/* --- GRÁFICO DE ONDA COM LAYOUT CENTRALIZADO --- */}
                <div>
                    <div className="flex justify-center items-center space-x-4 mb-2">
                        ...
                        <div className="text-sm font-bold text-slate-300 flex items-center space-x-3 ">
                    ...
                    <ResponsiveContainer width="100%" height={120}>
                        <BarChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                            ...
                            <Bar dataKey="wave_height_sg" name="Height" unit="m">
                                {chartData.map((entry) => (
                ...
                {/* --- GRÁFICO DE VENTO COM LAYOUT CENTRALIZADO --- */}
                <div>
                     <div className="flex justify-center items-center space-x-4 mb-2">
                        <div className="flex items-center space-x-2">
                    ...
                    <ResponsiveContainer width="100%" height={120}>
                        <BarChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                            ...
                            <Tooltip content={<ForecastTooltip />} cursor={{ fill: 'rgba(45, 55, 72, 0.8)' }} />
                            <Bar dataKey="wind_speed_sg" name="Speed" unit=" m/s">
                                {chartData.map((entry) => (
                                    <Cell key={`cell-${entry.timestamp_utc}`} fill={entry.timestamp_utc === spotlightHour.timestamp_utc ? spotlightColor : primaryColor} />
                                ))}
                            </Bar>
                ...
                {/* --- GRÁFICO DE MARÉ COM LAYOUT CENTRALIZADO --- */}
                <div>
                     <div className="flex justify-center items-center space-x-4 mb-2">
                        <div className="flex items-center space-x-2">
                    ...
                    <ResponsiveContainer width="100%" height={120}>
                        <BarChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                            ...
                            <Tooltip content={<ForecastTooltip />} cursor={{ fill: 'rgba(45, 55, 72, 0.8)' }} />
                            <Bar dataKey="sea_level_sg" name="Level" unit="m">
                                {chartData.map((entry) => (
                                    <Cell key={`cell-${entry.timestamp_utc}`} fill={entry.timestamp_utc === spotlightHour.timestamp_utc ? spotlightColor : primaryColor} />
                                ))}
                            </Bar>
                ...
                {/* --- GRÁFICO DE TEMPERATURA COM LAYOUT CENTRALIZADO --- */}
                <div>
                    <div className="flex justify-center items-center space-x-4 mb-2">
                        <div className="flex items-center space-x-2">
                            ...
                            <h4 className="text-sm font-bold text-slate-300 uppercase tracking-wider">Temperature</h4>
                        </div>
                        <div className="text-sm font-bold text-slate-300 flex items-center space-x-3 ">
                    ...
                    <ResponsiveContainer width="100%" height={120}>
                        <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                            ...
                            <Bar dataKey="water_temperature_sg" name="Water" unit="°C" fill="#3b82f6" barSize={12}>
                                {chartData.map((entry) => (
                                    <Cell key={`cell-water-${entry.timestamp_utc}`} fill={entry.timestamp_utc === spotlightHour.timestamp_utc ? spotlightColor : primaryColor} />
                                ))}
                            </Bar>
                            <Bar dataKey="air_temperature_sg" name="Air" unit="°C" fill="#bea925" barSize={12}>
                                {chartData.map((entry) => (
