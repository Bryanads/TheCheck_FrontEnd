...
import { WaveIcon, WindIcon, ChartBarIcon } from '../icons'; // Importando ícones para os títulos
    ...
    return (
        <div className="w-full h-[550px] bg-slate-800 rounded-lg p-6 overflow-x-auto overflow-y-hidden">
            <div style={{ width: `${allHourlyData.length * 40}px`, height: '100%' }}>
                {[
                    { key: 'swell_height_sg', name: 'Onda', unit: 'm', color: '#06b6d4', icon: <WaveIcon /> },
                    { key: 'wind_speed_sg', name: 'Vento', unit: 'm/s', color: '#eab308', icon: <WindIcon /> },
                    { key: 'sea_level_sg', name: 'Maré', unit: 'm', color: '#22c55e', icon: <ChartBarIcon /> }
                ].map((chart, index) => (
                    <div key={chart.key}>
                        {/* ADIÇÃO DO TÍTULO AQUI */}
                        <div className="flex items-center space-x-2 mb-1 pl-8">
                            <div className="w-5 h-5 text-slate-400">{chart.icon}</div>
                            <h3 className="text-md font-bold text-slate-300">{chart.name} ({chart.unit})</h3>
                        </div>
                        <ResponsiveContainer width="100%" height={index === 2 ? 180 : 150}>
                            <BarChart data={allHourlyData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }} onClick={handleChartClick}>
                                ...
                                {allHourlyData.map((d, idx) => {
                                    if (d.isSeparatorLine) {
                                        return <ReferenceLine key={`separator-${idx}`} x={idx} stroke="rgba(0,0,0,1)" strokeWidth={5} />;
                                    }
                                    return null;
                                })}
                            </BarChart>
